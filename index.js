...
async function verificarTrafego() {
  ...
  try {
    ...
    console.error("❌ Erro ao verificar o tráfego:", error.response?.data || error.message);
  }
}
...
async function enviarAlerta(mensagem) {
  ...
  try {
    ...
    console.error("❌ Erro ao enviar alerta para o Telegram:", error.response?.data || error.message);
  }
}
